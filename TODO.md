...
* [x] AI Suggestions — ✨ button opens panel; category-first two-step flow (6 categories → 15 city-specific results); checkbox selection; bulk insert into locations table; extractCity() uses city→region→country priority; 30-min sessionStorage cache; richer cards (priceRange badge, whyVisit, mustTry, bestTimeToVisit, tip)
* [ ] Better Place cards
* [ ] Improve mobile layout

## Itinerary

* [x] AI Suggestions — ✨ button (shown when days exist); category-first two-step flow (6 activity categories → 15 city-specific results); checkbox selection; day selector (auto if 1 day, dropdown if multiple); bulk insert into itinerary_items; 30-min sessionStorage cache; richer cards (suggestedTime, duration, tip)
